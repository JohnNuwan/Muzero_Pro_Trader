...
import numpy as np
import pandas as pd
from gemini_v19.environment.deep_trinity_env import DeepTrinityEnv
...
class CommissionTrinityEnv(DeepTrinityEnv):
    ...
    def step(self, action):
        ...
        # Asymmetric PnL Reward (Balanced)
        if realized_pnl > 0:
            reward += (realized_pnl / self.initial_balance) * 100.0
            self.total_profitable_trades += 1
        elif realized_pnl < 0:
            reward += (realized_pnl / self.initial_balance) * 150.0  # 1.5x Penalty (was 2x)
