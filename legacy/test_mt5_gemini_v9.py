...
# --- ⚙️ CONFIGURATION DU QUARTIER GÉNÉRAL (V9 - THE FORTRESS) ---
LOGIN = 1512117588
PASSWORD = "?9?j7fY3s$*!?*"
SERVER = "FTMO-Demo"
...
# --- 🛡️ GESTION DES RISQUES & COÛTS (V9) ---
# Configuration spécifique par actif pour optimiser les coûts et la volatilité
SYMBOL_PARAMS = {
    "EURUSD":     {"lot": 0.4,  "be": 30,   "pyr": 75,   "sl_mult": 1.5}, # Forex Standard
    "GBPUSD":     {"lot": 0.4,  "be": 30,   "pyr": 75,   "sl_mult": 1.5}, # Forex Standard
    "XAUUSD":     {"lot": 0.1,  "be": 300,  "pyr": 300,  "sl_mult": 2.0}, # Gold (Volatile)
    "BTCUSD":     {"lot": 0.1, "be": 50000, "pyr": 50000, "sl_mult": 3.0}, # Crypto (Spread/Comm élevés)
    "US30.cash":  {"lot": 1,  "be": 2100,  "pyr": 2500,  "sl_mult": 1.5}, # Indices
    "US100.cash": {"lot": 1,  "be": 2000,  "pyr": 2000,  "sl_mult": 1.5},
    "GER40.cash": {"lot": 1,  "be": 2100,  "pyr": 2100,  "sl_mult": 1.5},
    "US500.cash": {"lot": 1,  "be": 400,   "pyr": 400,   "sl_mult": 1.5},
}
...
DAILY_LOSS_LIMIT = -500.0 # 🛑 Hard Equity Stop (USD)
...
def check_environment(symbol):
    ...
    limit = MAX_SPREAD_POINTS if "US" or "GER40.cash" in symbol else 50 
...
# --- FEATURE ENGINEERING AVANCÉ (V8/V9) ---
...
# --- 📰 FILTRE NEWS ÉCONOMIQUES (V9) ---
# NOTE: La fonction calendar_events n'est pas disponible dans l'API Python MT5 standard.
# Pour l'instant, le filtre est désactivé. Il faudrait utiliser une API externe (ForexFactory, etc.)

_news_filter_enabled = False # Désactivé par défaut car pas de source de données

def update_news_cache():
    """Placeholder pour future intégration API News"""
    pass

def check_news(symbol):
    """
    Vérifie si on est dans une zone de danger.
    Actuellement désactivé (Retourne toujours False).
    """
    return False, ""
...
def normalize_volume(symbol, volume):
    """Normalise le volume selon les contraintes du courtier"""
    info = mt5.symbol_info(symbol)
    if info is None: return volume
...
def execute_trade(symbol, signal, confidence, df, scaler, model):
    ...
    # Cas A: Tendance Forte (The Fortress)
    trend_confirm = False
    if signal == 1 and htf_trend == "STRONG BUY": trend_confirm = True
    if signal == 0 and htf_trend == "STRONG SELL": trend_confirm = True
    
    # Cas B: Mean Reversion (The Sniper) - Contre-tendance autorisé si excès statistique
    mean_reversion = False
    if z_score < -2.5 and signal == 1: mean_reversion = True # Oversold -> Buy
    if z_score > 2.5 and signal == 0: mean_reversion = True  # Overbought -> Sell
    
    # Décision Finale
    if not trend_confirm and not mean_reversion:
        # Si ni tendance forte, ni excès statistique -> On filtre
        return
    ...
    if len(v8_positions) > 0:
        ...
        if profit_points > pyr_threshold and (confidence + penalty) > 0.70 and len(v8_positions) < 3:
            ...
            if (last_pos.type == 0 and signal == 1) or (last_pos.type == 1 and signal == 0):
                raw_lot = params["lot"] * 0.5
                lot = normalize_volume(symbol, raw_lot)
                ...
                request = {
                    ...
                    "tp": tp,
                    "magic": 888888,
                    "comment": f"Gemini V9 (Pyramiding #{len(v8_positions)})",
                ...
                if result is None:
                    ...
                    console.print(f"[red]Echec Pyramiding: Result is None. MT5 Error: {last_error}[/red]")
                    return
                ...
                if result.retcode == mt5.TRADE_RETCODE_DONE:
                    ...
                    # Telegram Enhanced
                    strat_type = "TREND" if trend_confirm else "SNIPER (Rev)"
                    tg_msg = (
                        ...
                        f"Trend: `{htf_trend}`\n"
                        f"News: `{'SAFE' if not is_news else 'DANGER'}`\n"
                        f"Conf: `{confidence:.1%}`"
                    )
    ...
    request = {
        ...
        "tp": tp,
        "magic": 888888,
        "comment": f"Gemini V9 ({confidence:.2f})",
        "type_time": mt5.ORDER_TIME_GTC,
        "type_filling": mt5.ORDER_FILLING_IOC,
    }
    ...
    if result is None:
        last_error = mt5.last_error()
        console.print(f"[red]Echec Trade: Result is None. MT5 Error: {last_error}[/red]")
        return
    ...
    try:
        ...
        for deal in deals:
            ...
            if deal.entry == mt5.DEAL_ENTRY_OUT: # C'est une sortie (fermeture)
                ...
                # Pips Calculation (Approximation via Price)
                # Il faudrait matcher avec l'entry deal pour être précis, mais on peut estimer
                # Ou juste utiliser le profit / valeur du point (moins précis si volume change)
                # Pour l'instant on se contente du profit USD et Win/Loss qui sont fiables
...
def manage_positions():
    ...
    for pos in positions:
        ...
        params = SYMBOL_PARAMS.get(symbol, SYMBOL_PARAMS["EURUSD"])
        be_trigger = params["be"] # Seuil dynamique (ex: 5000 pour BTC, 50 pour EURUSD)
            ...
            if not is_secured:
                ...
                if close_vol >= min_vol:
                    req_close = {
                        ...
                        "magic": 888888,
                        "comment": "Gemini V9 Partial Close",
                        "type_time": mt5.ORDER_TIME_GTC,
                        "type_filling": mt5.ORDER_FILLING_IOC,
                    }
                ...
                # Buffer dynamique : Spread + 1 pip (10 points)
                info = mt5.symbol_info(symbol)
                spread_points = info.spread * point
                be_buffer = spread_points + (10 * point)
                
                if "BTC" in symbol: be_buffer = max(be_buffer, 100 * point) # Safety for BTC
...
if __name__ == "__main__":
    ...
    console.print("\n")
    console.print(Panel.fit("🏰 [bold cyan]GEMINI V9 - THE FORTRESS[/bold cyan]", border_style="cyan"))
    ...
    while True:
        ...
        if daily_profit < DAILY_LOSS_LIMIT:
            ...
            time.sleep(3600) # Pause 1h avant de revérifier (ou exit)
            continue
        ...
        console.print(Panel(
            ...
            title="[bold magenta]GEMINI V9 DASHBOARD[/bold magenta]",
        ...
        for sym in SYMBOLS:
            ...
            try:
                ...
                # News Check
                is_news, news_msg = check_news(sym)
                if is_news:
                    status = f"[bold red]{news_msg}[/bold red]"
                else:
                    status = "[dim]Wait[/dim]"
                ...
                if positions and len(positions) > 0:
                    ...
                    # On laisse execute_trade décider si c'est Trend ou Sniper
                    # Mais on affiche ENTRY si l'un des deux est validé
                    # Pour l'affichage simple, on met ENTRY si conf ok, execute_trade filtrera
