...
class AlphaZeroMCTS:
    ...
    def _expand_root(self, root):
        ...
        with torch.no_grad():
            policy_logits, _ = self.network(torch.FloatTensor([root.state]))
            policy = policy_logits[0].numpy() # Softmax already applied in net? Yes.
    ...
    def _evaluate(self, node, sim_env):
        ...
        with torch.no_grad():
            policy_logits, value = self.network(torch.FloatTensor([node.state]))
