...
import os
import random
from MuZero.config_v2 import MuZeroConfigV2
...
def train_muzero_v2():
    ...
    print(f"🚀 Starting MuZero V2 Training (FTMO Optimized)")
    print(f"   Learning Rate: {config.learning_rate} (5× V1)")
    print(f"   Quality Bonus: {config.quality_trade_bonus}× (Trade +1% = +5pts)")
    print(f"   Final Growth Bonus: +{config.final_growth_bonus} pts (if >10% growth)")
    ...
    for symbol in config.symbols:
        try:
            ...
            envs[symbol] = CommissionTrinityEnv(
                symbol=symbol,
                lookback=1000,
                quality_trade_multiplier=config.quality_trade_bonus,
                enable_final_growth_bonus=True,
    ...
    buffer_save_path = os.path.join("MuZero/results", "replay_buffer_initial.pkl")
    if os.path.exists(buffer_save_path):
        print(f"📦 Loading V1 replay buffer to bootstrap V2...")
        import pickle
        with open(buffer_save_path, 'rb') as f:
            agent.replay_buffer = pickle.load(f)
        print(f"✅ Loaded {len(agent.replay_buffer.buffer)} games from V1!")
        ...
        if step % 5 == 0:
            ...
            if len(logger.metrics["avg_reward"]) > 0:
                ...
                if current_mean_reward > best_mean_reward:
                    ...
                    torch.save(agent.network.state_dict(), os.path.join(config.weights_path, "best_model_v2.pth"))
                    print(f"🔥 V2 New Best Model Saved! Reward: {best_mean_reward:.2f}")
